...
(defun livedown-preview ()
  ...
  (call-process-shell-command
   (format "livedown stop --port %s &" livedown-port))
  ...
  (print (format "%s rendered @ %s" buffer-file-name livedown-port)
...
(defun livedown-kill (&optional async)
  ...
  (interactive)
  (let ((stop-livedown
         (if async 'async-shell-command 'call-process-shell-command)))
